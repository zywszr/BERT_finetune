    ...
    for epoch in range(args.epochs):
        ...
        with tqdm(total=len(train_dataloader), desc=f"Epoch {epoch}") as pbar:
            for step, batch in enumerate(train_dataloader):
                ...
                loss, prediction_score, seq_relationship_score = outputs[0:3]
                if args.debug:
                    lm_label_ids = lm_label_ids.view(-1)
                    pos = lm_label_ids != -1
                    _, prediction = torch.max(prediction_score.view(-1, model.config.vocab_size)[pos], 1)
                    print(prediction)
                    prec1 = torch.sum(lm_label_ids[pos] == prediction).item() / torch.sum(pos).item()
                    print(torch.sum(lm_label_ids[pos] == prediction).item(), torch.sum(pos).item())
                    is_next = is_next.view(-1)
                    _, prediction = torch.max(seq_relationship_score.view(-1, 2), 1)
                    print(prediction)
                    prec2 = torch.sum(is_next == prediction).item() / is_next.shape[0]
                    print(torch.sum(is_next == prediction).item(), is_next.shape[0])

                    # _, prediction = torch.max(outputs[1].view(-1, model.config.vocab_size), 1)
                    print(prec1, prec2)
                    return
                if n_gpu > 1:
